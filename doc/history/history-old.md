...
### 執行命令
```powershell
azd init --environment {environment-name}
...
| Cosmos DB | cosmos-{token} |
| Container Apps Environment | cae-{token} |
| Azure AI Search | srch-{token} |
| Container App (Frontend) | ca-{token}-frontend |
| Container App (MCP) | ca-{token}-mcp |
| Container App (Data Ingestion) | ca-{token}-dataingest |
| Container App (Orchestrator) | ca-{token}-orchestrator |
| AI Foundry | aif-{token} |
| AI Key Vault | kvf-{token} |
| Chat Model Deployment | gpt-4.1 (2025-04-14) |
| Embedding Model Deployment | text-embedding-3-large |
| AI Foundry Cosmos DB | aiservices-{token}_cosmos |
| AI Foundry Search | srch-aif{token} |
| AI Foundry Storage | aif{token} |
| AI Foundry Project | project-{token} |
| AI Foundry Capability Host | caphost-{token} |
...
| Name | Status | Image |
|------|--------|-------|
| ca-{token}-frontend | Running | cr{token}.azurecr.io/azure-gpt-rag/frontend:85f9446 |
| ca-{token}-orchestrator | Running | cr{token}.azurecr.io/azure-gpt-rag/orchestrator:latest |
| ca-{token}-mcp | Running | cr{token}.azurecr.io/azure-gpt-rag/mcp:latest |
| ca-{token}-dataingest | Running | cr{token}.azurecr.io/azure-gpt-rag/dataingest:latest |
...
### 前端 URL
🌐 **https://ca-{token}-frontend.calmcoast-6a1d388b.eastus2.azurecontainerapps.io**

### 重要端點
| 服務 | URL |
|------|-----|
| Frontend | https://ca-{token}-frontend.calmcoast-6a1d388b.eastus2.azurecontainerapps.io |
| Orchestrator | https://ca-{token}-orchestrator.calmcoast-6a1d388b.eastus2.azurecontainerapps.io |
| MCP | https://ca-{token}-mcp.calmcoast-6a1d388b.eastus2.azurecontainerapps.io |
| Data Ingestion | https://ca-{token}-dataingest.calmcoast-6a1d388b.eastus2.azurecontainerapps.io |
| App Configuration | https://appcs-{token}.azconfig.io |
| AI Foundry | https://aif-{token}.openai.azure.com/ |
| Container Registry | cr{token}.azurecr.io |
...
```powershell
# 建置映像
az acr build --registry cr{token} --image gpt-rag-orchestrator:conv-memory-v2-20260113 --file Dockerfile .

# 部署至 Container App
az containerapp update --name ca-{token}-orchestrator --resource-group {resource-group} --image cr{token}.azurecr.io/gpt-rag-orchestrator:conv-memory-v2-20260113
```
...
#### 部署狀態
⚠️ **待部署** - 需要重新建立 Resource Group (已刪除 `{resource-group}`，但無權限建立新 RG)

#### 後續步驟
1. 請管理員建立 Resource Group `{resource-group}` (eastus2)
2. 執行 `azd up` 重新部署
...
#### 執行命令
```powershell
az containerapp update --name ca-{token}-mcp --resource-group {resource-group} --min-replicas 0 --max-replicas 0
```

#### 恢復方式
```powershell
az containerapp update --name ca-{token}-mcp --resource-group {resource-group} --min-replicas 1 --max-replicas 1
...
| 項目 | 狀態 |
|------|------|
| Resource Group `{resource-group}` | ❌ 不存在/無權限 |
